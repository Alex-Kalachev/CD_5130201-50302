...
int main()
{
     ...
     * bottom = -190087072
     * 
     * Поля класса были проинициализированны мусором
     *
     ...
     * 
     * При выполнении следующего блока будет вызываться код сгенерирован компилятором автоматически:
     * 
     * Rect::Rect()
     * Rect::~Rect()
     * 
     * (Так как явно не были написаны конструкторы и деструкторы)
     * 
     * В этом случае все поля класса - встроенные типы, поэтому они остаются непроинициализированными, 
     * и после завершающей этот блок фигурной скобки вызывается деструктор по умолчанию (в данном случае ничего не делает)
     * 
     * В общем случае:
     * 
     * Конструктор по умолчанию для
     * - поля-объекта: вызывает его конструктор
     * - базового типа: оставляет его не инициализированным
     * 
     * Деструктор по умолчанию для
     * - вызываются деструкторы базовых классов
     * - вызывает деструкторы всех полей-объектов
     */
     ...
     * конструктора.
     * 
     * [ для класса "Rect" не существует конструктор по умолчанию ]
     * 
     * так как мы создали пользовательский конструктор (с параметрами), конструктор по умолчанию не генерируется автоматически компилятором
     * а для вида Rect recе; конструтор (без параметоров) не объявлен.
     *
     ...
     * 
     * В этом случае:
     * - пользовательский конструтор с параметрами: принималет 4 координаты сторон прямоугольника (соответственно полям)
     * - конструктор копирования по умолчанию: копирует значения полей объекта из одного в другой (путем побитогого копирования)
     * 
     * В общем случае Rect::Rect(const Rect &other){}:
     * - копирует все поля класса
     *    * для встроенных типов производит побитовое копирование или, например, для указателя записывает значение адреса того же места в памяти на кторое указвает указатель объекта класса, у которго мы копируем значения
     *    * для поля-объекта вызывается его конструктор копирования
     *    * для базового класса вызывается его конструктор копирования
     */

    {
        Rect rect1(1, 2, 3, 4); // вызывается конструктор пользовательского типа (с параметрами) 
        Rect rect2 = rect1; // вызывается конструктор копирования по умолчанию автоматически сгенерированный компилятором
        Rect rect3(rect1); // аналогично вызывается конструктор копирования по умолчанию автоматически сгенерированный компилятором
    } // 1.5 вызывается деструктор *3
     ...
     * случае?
     *
     * Деструктор - это специальный метод класса, который выполняется при уничтожении объекта.
     * Он вызывается:
     * 1. именованный автоматический объект - при выходе из блока, в ктором он был объявлен
     * 2. встроенный массив объектов - аналогично
     * 3. динамический объект - когда программист явно его уничтожает
     * 4. глобальный, в пространстве имен - по завершению программы
     * 5. статический локальный - при завершении программы
     * 6. временный - по завершению вычисления всего выражения
     * 
     * В моем классе Rect есть деструткор который вызывается по умолчанию (автоматически генерируется компилятором)
     * 
     * В этом случае:
     *      ничего не делает
     * 
     ...
     * 12 констр
     * 11 дестр
     * для static объекта деструктор вызовется только в конце программы
     */

    {
        ...
        Rect *pR = new Rect(1,2,1,2); // вызывается конструктор с параметрами
        {
            ...
            Rect arRect[2]; // вызывается конструктор по умолчанию (2 штуки)
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ; // вызывается конструктор с параметрами 
                Rect r4(*pR); // вызывается конструктор копирования *3
                Rect r5(i,i,i,i); // вызывается конструктор с параметрами *3 
            } // вызов двух деструторов (r4, r5) *3
        } // вызов деструтора для arRect[1], arRect[0], r2
        delete pR;  // вызов деструтора для pr
    } //вызов деструтора для r1
    ...
    {
        Rect r1;
        // std::cout << r1.m_left << std::endl;
    } 

    // [ error: 'm_left' is a private member of 'Rect' ]
    // мы пытаемся обратиться к полю объекта, кторое находится под спецификатором доступа private (они доступны только внутри методов этого класса)
     ...
     * 1. Она объявлена внутри класса и имеет доступ к его приватным и защищённым полям.
     * 2. Вызывается через объект класса (например, r1.getleft()).
     * 3. Может использовать ключевое слово this, чтобы ссылаться на конкретный объект ( то есть скрыто принимает адрем на объект).
     */
    ...
    {
        Rect r1(1,2,3,4), r2(-1,-2,-3,-4); // вызвано два конструтора с параметрами
        Rect r3 = bounding_rect(r1, r2); // вызвано два  конструктора копирования 
        print_rect(r3);
    } 
...

    {
        ...
        print_rect(r);
        std::cout << "Ширина: "  << r.get_width() << "\n";
        std::cout << "Высота: "   << r.get_height() << "\n";
        std::cout << "Площадь: "  << r.get_square() << "\n\n";

        
        r.set_width(10);   
        r.set_height(8);
        std::cout << "Измененный прямоугольник:\n";
        print_rect(r);
    ...
    {
        Bochka alch(100, 96); // бочка со спиртом
        Bochka water(100, 0);

        int count = 0;

        while(alch.сoncentration() > 50)
        {
            alch.iteration(water);
            water.iteration(alch);

            count += 1;

        }

        std::cout << "Число итераций:\t" << count << "\n";
    } 
    ...
    {
        Matrix fib(2,2,1);     
        fib.set(1,1,0);//[[1, 1], [1, 0]]
        Matrix curr(2, 1, 1);
        curr.set(1, 0, 0);//[F1, F0] = [1, 0]
        ...
        {
            printf("%d число: %.10g\n", i + 1, curr.get(0, 0));
            Matrix temp = fib.multiply(curr);
            curr.set(0, 0, temp.get(0, 0));
            curr.set(1, 0, temp.get(1, 0));
        }
...

    {
        ...
        s1.set_new_string("Это новоя строка.");
        s1.print();
...

    {
        ...
        SpeechBubble bubble(user_input.get_data(), 40);
